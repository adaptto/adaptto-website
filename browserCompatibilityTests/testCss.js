...
(async () => {
  ...
  const cssFiles = await globby(['styles/*.css', 'blocks/**/*.css']);
  for (const cssFile of cssFiles) {
    let filenamePrinted = false;
    const css = await readFileAsync(cssFile, 'utf8');
    ...
    for (const browser of BROWSERS) {
      ...
      if (css === result.css) continue; // css didn't need any modification to support the browsers
      if (!filenamePrinted) {
        console.log('\n########################################################');
        console.log(basename(cssFile));
        console.log('########################################################');
      ...
      const diff = Diff.diffCss(css, result.css);
      ...
      diff.forEach((part) => {
        ...
        const color = part.added
          ...
          : part.removed
            ...
            : COLORS.gray;
        let { value } = part;
        // only show couple of lines of the original css when not modified
